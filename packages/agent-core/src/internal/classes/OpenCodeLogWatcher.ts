  ...
  static getErrorMessage(error: OpenCodeLogError): string {
    ...
    switch (error.errorName) {
      ...
      case 'AI_APICallError':
        if (
          messageLower.includes('usage limit has been reached') ||
          messageLower.includes('usage_limit_reached')
        ) {
          return 'Provider usage limit reached. Update billing or switch to another provider/model in Settings, then retry the task.';
        }
