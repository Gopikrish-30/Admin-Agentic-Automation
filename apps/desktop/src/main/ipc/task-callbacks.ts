...
const MOCK_ADMIN_ONLY_ALLOWED_TOOL_BASENAMES = [
  'start_task',
  'complete_task',
  'todowrite',
  'ask_user_question',
...
const MOCK_ADMIN_ONLY_ALLOWED_TOOL_PREFIXES = ['dev-browser-mcp_', 'dev_browser_mcp_', 'browser_'];
const MOCK_ADMIN_ONLY_GUARD_ENABLED =
  process.env.NAVIGATOR_MOCK_ADMIN_ONLY !== '0' && process.env.NAVIGATOR_MOCK_ADMIN_ONLY !== 'false';
...
function isAllowedInMockAdminOnly(toolName: string): boolean {
  const normalizedToolName = normalizeToolName(toolName);
  if (
    MOCK_ADMIN_ONLY_ALLOWED_TOOL_BASENAMES.some((name) =>
      matchesToolNameOrSuffix(normalizedToolName, normalizeToolName(name)),
    )
  ) {
    return true;
  }
  return MOCK_ADMIN_ONLY_ALLOWED_TOOL_PREFIXES.some((prefix) =>
    normalizedToolName.startsWith(normalizeToolName(prefix)),
  );
}
...
export function createTaskCallbacks(options: TaskCallbacksOptions): TaskCallbacks {
  ...
  let hasMockAdminViolation = false;
  ...
  return {
    ...
    onToolCallComplete: ({ toolName, toolOutput }) => {
      if (MOCK_ADMIN_ONLY_GUARD_ENABLED && !isAllowedInMockAdminOnly(toolName)) {
        if (hasMockAdminViolation) {
          return;
        }

        hasMockAdminViolation = true;
        const reason =
          `Blocked disallowed tool in mock-admin-only mode: ${toolName}. ` +
          'Only browser automation tools are permitted.';
        ...
        void taskManager.cancelTask(taskId).catch((error) => {
          const errorMessage = error instanceof Error ? error.message : String(error);
          console.warn('[TaskCallbacks] Failed to cancel task after mock-admin violation:', errorMessage);
        });
