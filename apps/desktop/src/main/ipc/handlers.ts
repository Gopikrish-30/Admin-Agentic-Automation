...
function buildMockAdminOnlyPrompt(userPrompt: string): string {
  return [
    ...
    'Rules:',
    '- Use only browser automation tools as a human operator would in the UI.',
    '- Do not use local filesystem automation, shell commands, direct APIs, or direct DOM shortcuts.',
    '- Restrict actions to create/update/delete/reset password/license management in the mock admin panel.',
    '',
  ...
  handle('logs:export', async (event: IpcMainInvokeEvent) => {
    ...
    try {
      if (fs.existsSync(logPath)) {
        ...
        const header = `Admin Application Logs\nExported: ${new Date().toISOString()}\nLog Directory: ${logDir}\n\nNo logs recorded yet.\n`;
